...
public class SimpleJsonWriter {
    ...
    public void save(JsonObject parent, JsonEntity value, Writer tw, int level, JsonEntry member, String separator, boolean noIndent) throws IOException {
        ...
        Pair<String[], CommentType> comment = comment(parent, member);
            ...
            case BOOLEAN:
                ...
                if (comment != null) {
                    if (comment.getValue().equals(CommentType.BEHIND_VALUE)) {
                        ...
                        break;
                    } else if (comment.getValue().equals(CommentType.MULTI_LINE)) {
                        tw.write(val);
                        tw.write("  \n");
                        tw.write("  /*\n");
    ...
    public Pair<String[], CommentType> comment(JsonObject parent, JsonEntry member) {
    ...
    public void writeString(String value, Writer tw, int level, String separator) throws IOException {
        ...
        if (doEscape ||
                SimpleJsonParser.isWhiteSpace(left) || SimpleJsonParser.isWhiteSpace(right) ||
                left=='"' ||
                left=='\'' ||
                left=='#' ||
                left=='/' && (left1=='*' || left1=='/') ||
    ...
    public static boolean startsWithKeyword(String text) {
        ...
        while (p<text.length() && SimpleJsonParser.isWhiteSpace(text.charAt(p))) p++;
